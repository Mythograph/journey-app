...
import { savePurchase } from "../../src/lib/purchases.js";
...
const handler: Handler = async (event) => {
  ...
  try {
    ...
    // Write directly — avoids a second Blobs read (updatePurchase reads then writes)
    await savePurchase({ ...purchase, narrative: accumulated });
    console.log(`generate-bg: saved narrative for token ${purchase.token.slice(0, 10)}… (${accumulated.length} chars)`);
